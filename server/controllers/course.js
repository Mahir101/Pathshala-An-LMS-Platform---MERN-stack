...
export const paidEnrollment = async (req, res) => {
  try {
    ...
    const result = await User.findByIdAndUpdate(
      ...
      console.log('SEE HERE SEEEEEEEEEE HEEEEEEEEEERRRRRRRREEEEEEEEEEE!!!!!!!!!!')
    console.log(course.instructor)
    try{
      const user = await User.findByIdAndUpdate(
            course.instructor,
            { $inc: { money: req.params.amount } } 
          ).exec();
          console.log(user)
    ...
    //   
    //   }catch(err){
    //     console.log("Input in DB error", err);
    //     return res.status(401).send("Error in sending values in DB");
